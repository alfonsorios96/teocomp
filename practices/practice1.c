...
void p1_concat(char string1[], char string2[]){
	...
	str_concat(string2, string1, concatArray2);
...
void p1_subfix(char string1[], char string2[]){
	...
	printf("El subfijo de %d de '%s' es : '%s'\n", fixes , string1, subfixArray1);
...
void p1_pow(char s1[], char s2[]){
	int n;

	printf("\nIntroduce la potencia : ");
	scanf("%i", &n);

	char s3[str_size(s1) * n];
	char s4[str_size(s2) * n];

	str_pow(s1, s3, n);
	str_pow(s2, s4, n);

	printf("\nPotencia %i de %s : %s", n, s1, s3);
	printf("\nPotencia %i de %s : %s\n\n", n, s2, s4);
}